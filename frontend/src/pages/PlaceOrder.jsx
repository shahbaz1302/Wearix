...
const PlaceOrder = () => {
  ...
  const { theme, navigate, backendUrl, cartItems, setCartItems, token, getCartAmount, delivery_fee, products } = useContext(ShopContext)
    ...
    const options = {
      ...
      handler: async (response) => {
        ...
        if (data.success) {
          ...
          setCartItems({});
          navigate(
            `/verify?success=true&method=razorpay&orderId=${data.orderId}&payment=paid`
          );
        } else {
      ...
      switch (method) {
        case "cod":
          ...
          if (response.data.success) {
            setCartItems({})
            navigate(
              `/verify?success=true&method=cod&orderId=${response.data.orderId}&payment=pending`
            );
          }
          else {
  ...
  return (
    <form onSubmit={onSubmitHandler} className="flex flex-col sm:flex-row justify-between gap-4 pt-5 sm:pt-14 min-h-[80vh] border-t">
      ...
      <div className="flex flex-col gap-4 w-full sm:max-w-120">
        ...
        <div className="flex gap-3">
          <input required onChange={onChangeHandler} name="zipcode" value={formData.zipcode} className="border border-gray-300 rounded py-1.5 px-3.5 w-full" placeholder="Zip Code" type="numeric" pattern="[0-9]*" />
        ...
        <input required onChange={onChangeHandler} name="phone" value={formData.phone} className="border border-gray-300 rounded py-1.5 px-3.5 w-full" placeholder="Phone" type="numeric" pattern="[0-9]*" />
      </div>
        ...
        <div className="mt-12">
          ...
          <div className="flex gap-3 flex-col lg:flex-row">
            ...
            <div onClick={() => setMethod("cod")} className="flex items-center border gap-3 p-2 px-3 cursor-pointer">
              ...
              <p className="text-(--text1-color) text-sm font-medium mx-4">CASH ON DELIVERY</p>
            </div>
          </div>

          <div className="w-full text-end mt-8">
            <button type="submit" className={`${theme === "dark" ? "bg-white text-black" : "bg-black text-white"} px-16 py-3 text-sm cursor-pointer transition-colors duration-350`}>PLACE ORDER</button>
          </div>
